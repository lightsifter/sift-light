...
export async function buildSearchPlugin(root: string): Promise<void> {
  ...
  const identity = {
    ...
    description:
      "Require sift-light for conventional local searches while keeping development tools available.",
  ...
  const mcpEnvironment = {
    ...
    SIFT_LIGHT_MCP_OUTPUT_MODE: "model",
  };
  const mcpConnection = {
    command: "npx",
    args: [
      "--yes",
      "--package",
      `sift-light-runtime@npm:${packageJson.name}@${packageJson.version}`,
      "sift-light-mcp",
      "--stdio",
    ],
    env: mcpEnvironment,
  ...
  const mcp = {
    "sift-light": mcpConnection,
  };
  const sharedMcp = {
    "sift-light": {
      ...mcpConnection,
      env_vars: [SIFT_LIGHT_CONFIG_ENV, ...SEMANTIC_JUDGE_API_KEY_ENVS],
    },
  };
  ...
  const files: Record<string, unknown> = {
    ...
    "kimi.plugin.json": {
      ...
      mcpServers: mcp,
      systemPrompt:
        "Use sift-light for local content and filename searches. The plugin blocks conventional alternative search entries. Ordinary reads, edits, tests and builds remain available.",
      hooks: [
        {
          event: "PreToolUse",
