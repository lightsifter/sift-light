...
import {
  SIFT_LIGHT_DESCRIPTION,
  SIFT_LIGHT_MODEL_DESCRIPTION,
  siftLightSchema,
} from "./tool-schema.js";

export const SIFT_LIGHT_MCP_PATH = "/mcp";
...
const SIFT_LIGHT_OUTPUT_SCHEMA: Tool["outputSchema"] = {
  type: "object",
  properties: {
    text: {
      type: "string",
...
function siftLightTool(outputMode: SiftLightMcpOutputMode): Tool {
  ...
  if (outputMode === "structured") tool.outputSchema = SIFT_LIGHT_OUTPUT_SCHEMA;
  return tool;
}
...
export function createSiftLightMcpServer(
  ...
  const server = new McpServer(
    { name: "sift-light", version: SIFT_LIGHT_VERSION },
    {
      capabilities: { tools: {} },
...
function reportHttpFailure(response: ServerResponse, error: unknown): void {
  process.stderr.write(`sift-light MCP request failed: ${errorMessage(error)}\n`);
  if (!response.headersSent) writeJsonError(response, 500, "MCP request failed");
  else response.destroy();
}

function requestPath(request: IncomingMessage): string {
  return new URL(request.url ?? SIFT_LIGHT_MCP_PATH, "http://sift-light.local").pathname;
}
