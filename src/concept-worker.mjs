...
function conceptModelDirectory() {
  return resolve(process.env.SIFT_LIGHT_MODEL_DIR ?? join(homedir(), ".cache", "sift-light", "models"), CONCEPT_REVISION);
}
function conceptCacheDirectory() {
  return resolve(process.env.SIFT_LIGHT_MODEL_DIR ?? join(homedir(), ".cache", "sift-light", "models"), "concept-cache", `${CONCEPT_REVISION}-v${String(CONCEPT_CACHE_VERSION)}`);
}
async function verifyConceptModel(directory = conceptModelDirectory()) {
  for (const asset of CONCEPT_ASSETS) {
    ...
    try {
      ...
      throw new SiftLightError(`Local concept model is missing or invalid (${asset.path}); run sift-light-model --install-model explicitly`, { cause: error });
    }
  }
}
