...
function blockedMatch(
  ...
  const repair =
    recovered.kind === "concrete"
      ? `retry exactly once through sift-light (possibly MCP-prefixed) with ${recovered.request}`
      : `an equivalent request was not generated because ${recovered.reason}; manually translate the search, then retry exactly once through sift-light (possibly MCP-prefixed) with ${recovery(match.kind)}`;
  return {
    block: true,
