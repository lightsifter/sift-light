...
export const CONCEPT_TIMEOUT_ENV = "SIFT_LIGHT_CONCEPT_TIMEOUT_MS";
...
export async function verifyConceptModel(directory = conceptModelDirectory()): Promise<void> {
  for (const asset of CONCEPT_ASSETS) {
    ...
    try {
      ...
      throw new SiftLightError(
        `Local concept model is missing or invalid (${asset.path}); run sift-light-model --install-model explicitly`,
        { cause: error },
      );
    }
  }
