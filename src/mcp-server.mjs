...
var package_default = {
  ...
  homepage: "https://github.com/lightsifter/sift-light#readme",
  bugs: {
    url: "https://github.com/lightsifter/sift-light/issues"
  },
  license: "AGPL-3.0-only",
  author: "baoer",
  repository: {
    type: "git",
    url: "git+https://github.com/lightsifter/sift-light.git"
  },
  bin: {
    "sift-light-mcp": "src/mcp-server.mjs",
    "sift-light-model": "src/concept-worker.mjs"
  },
  files: [
    "src/**/*.ts",
  ...
  omp: {
    extensions: [
      "./plugins/sift-light/omp-extension.mjs"
    ]
  },
  pi: {
    extensions: [
...
var SIFT_LIGHT_VERSION = package_default.version;
...
function modeOf(input) {
  ...
  if (typeof mode !== "string")
    return;
  return SIFT_LIGHT_MODES.find((candidate) => candidate === mode);
}
...
function validateRequestContract(input) {
  ...
  if (!mode)
    throw capabilityError("E_MODE_UNKNOWN", inputModeLabel(raw), "mode", `mode must be one of ${SIFT_LIGHT_MODES.join(", ")}`, "Choose one of the advertised modes and retry");
...
function schemaContractError(value, field, message) {
  ...
  }, `Invalid sift-light arguments at /${normalized}: ${message}`);
}

// src/model-error.ts
var MAX_RAW_ERROR_SCAN_CHARACTERS = 4096;
var MAX_MODEL_ERROR_CHARACTERS = 1024;
var MODEL_ERROR_PREFIX = "sift-light failed:";
...
import { dirname as dirname3, join as join4, resolve as resolve12 } from "node:path";
...
async function filterHistoricalPaths(cwd, paths, request, signal) {
  ...
  const root = await mkdtemp(join3(tmpdir(), "sift-light-paths-"));
...
class SourceAccess {
  ...
  async#readOnce(path, expected, verification) {
    ...
    if (verification)
      this.#budget.verificationBytes += document.bytes.length;
    else
      this.#budget.readBytes += document.bytes.length;
...
import { dirname as dirname4, extname as extname3, resolve as resolve21 } from "node:path";
...
var siftLightSchema = Type.Object({
  ...
  mode: Type.Optional(stringEnum(SIFT_LIGHT_MODES, {
...
function siftLightTool(outputMode) {
  const tool = {
    name: "sift-light",
    title: "sift-light",
    description: outputMode === "model" ? SIFT_LIGHT_MODEL_DESCRIPTION : SIFT_LIGHT_DESCRIPTION,
...
function createSiftLightMcpServer(service, cwd, outputMode = DEFAULT_MCP_OUTPUT_MODE) {
  ...
  const server = new McpServer({ name: "sift-light", version: SIFT_LIGHT_VERSION }, {
    capabilities: { tools: {} },
...
function reportHttpFailure(response, error) {
  process.stderr.write(`sift-light MCP request failed: ${errorMessage2(error)}
`);
  if (!response.headersSent)
...
function requestPath(request) {
  return new URL2(request.url ?? SIFT_LIGHT_MCP_PATH, "http://sift-light.local").pathname;
}
...
// src/mcp-cli.ts
var SIFT_LIGHT_MCP_USAGE = `Usage: sift-light-mcp [--http | --stdio]
...
async function runHttpServer(outputMode, semanticJudge, vectorSearchEnabled) {
  ...
  process.stderr.write(`sift-light MCP listening on http://${displayHost}:${String(address.port)}${SIFT_LIGHT_MCP_PATH}
`);
